...
public class PasswordGenerator {
	 private static final String CHAR_LOWER = "abcdefghijklmnopqrstuvwxyz";
	    ...
	    private static final String IDENTIFIER_ALLOW_BASE = CHAR_LOWER + CHAR_UPPER + NUMBER;
...
//	    }
	    ...
	    public static String generateRandomIdentifiers(int length) {
	        if (length < 1) throw new IllegalArgumentException();

	        StringBuilder sb = new StringBuilder(length);
	        for (int i = 0; i < length; i++) {

	            int rndCharAt = random.nextInt(IDENTIFIER_ALLOW_BASE.length());
	            char rndChar = IDENTIFIER_ALLOW_BASE.charAt(rndCharAt);
