...
export const MultiRangeSlider: FC<MultiRangeSliderProps> = ({
  ...
  useEffect(() => {
    ...
    parentCallback!({ title: title, operation: operation, minVal: valueTextLeft, maxVal: maxVal });

  }, [valueTextLeft]);

  useEffect(() => {
    // console.log("Effect Right");
    parentCallback!({ title: title, operation: operation, minVal: minVal, maxVal: valueTextRight });
  }, [valueTextRight]);
